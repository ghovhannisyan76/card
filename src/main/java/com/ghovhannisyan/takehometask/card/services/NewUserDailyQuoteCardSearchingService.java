...
@Component
public class NewUserDailyQuoteCardSearchingService implements CardSearchingService {
    ...
    public CardEntity search() {
        Optional<CardEntity> optionalCard = cardRepository.findByCardTypeAndCardShowDate(CardType.DAILY_NEW, LocalDate.now());
        if(!optionalCard.isPresent()) {
            throw new CardApiException(ErrorMessage.NOT_FOUND, "Daily quote card is not found for the new user", "internal");
        }
